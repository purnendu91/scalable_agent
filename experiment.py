...
flags.DEFINE_string('learner_host', 'localhost:8027',
                    'learner host, only one allowed.')
# flags.DEFINE_string('actor_hosts', 'localhost:9127,localhosts:9128',
#                     'actor hosts.')
flags.DEFINE_enum('actor_on', 'cpu', ['gpu', 'cpu'],
                  'actor on which device')
flags.DEFINE_string('actor_hosts', 'localhost',
                    'actor hosts')
flags.DEFINE_integer('actor_min_port', 9001,
                     'actor hosts min port.')
flags.DEFINE_integer('actor_max_port', 9012,
                     'actor hosts max port.')
...
def train(action_set, level_names):
  ...
  with tf.Graph().as_default(), \
       tf.device(local_job_device + '/' + FLAGS.actor_on), \
